...
**第二层：Browser fallback**
当普通抓取失败时（例如页面需要 JavaScript 渲染才能显示岗位），程序自动改用无头浏览器加载页面并读取渲染结果。这一层同样自动完成，不需要你做任何操作。

**第三层：Manual cURL fallback**
如果网站的数据只能通过 API / XHR 请求获取，前两层可能都拿不到数据。此时可以从浏览器开发者工具把请求复制为 cURL，交给 Job Extractor 继续采集：
...
> 从浏览器复制的 cURL 可能包含 Cookie、Token、`Authorization`、Session 等敏感信息，不要直接上传到 GitHub、Issue、日志或公开聊天；分享前先检查并删除这些请求头（详见第 19 节安全提醒）。
